<template>
  <client-only>
    <div :id="`ProductCard${product.id}`"
      class="h-[100%] w-[100%] bg-white border inline-block rounded hover:shadow-[0_0_10px_3px_rgba(0,0,0,0.15)] cursor-pointer">
      <NuxtLink :to="`/item/${product.id}`" class="flex flex-col justify-between">
        <div class="h-[160px]">
          <NuxtImg :src="product.url" class="rounded-t h-[100%] w-full"
        ...
        <div id="ProductDetails" class="p-2">
          ...
          <p class="flex items-center px-1 pt-0.5 text-xs text-[#252525]">
            5,000+ sold
            <Icon name="material-symbols:star-rate" color="#757575" class="ml-1.5" />
            4.7
          </p>
